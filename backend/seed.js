...
// Avoid model overwrite errors if it already exists in the Mongoose cache
...
const full80QuestionsDataset = [
    ...
    // ==========================================
    { category: "Cybersecurity", questionText: "What mechanism provides secure encrypted transmission tunnels across open public network routes?", options: ["Firewall Configuration", "Virtual Private Network (VPN)", "Proxy Server Relay", "DNS Security Extensions"], correctAnswer: "Virtual Private Network (VPN)" },
    ...
    { category: "Cybersecurity", type: "text", questionText: "What type of threat sits quietly inside a useful program before running its payload?", options: ["Worm Propagation", "Trojan Horse", "Spyware Keylogger", "Rootkit Command Controller"], correctAnswer: "Trojan Horse" },
...
async function seedNow() {
    try {
        ...
        if (!process.env.MONGODB_URI) {
            console.error("❌ ERROR: Your .env file is missing 'MONGODB_URI' or has a name typo!");
            process.exit(1);
        }
        ...
        console.log("🧹 Flushed existing records safely.");

        console.log("🚀 Inserting 80 production quiz items...");
        await Question.insertMany(full80QuestionsDataset);
        
        console.log("✅ SUCCESS! All 80 production questions are now completely live on MongoDB!");
        process.exit(0);
    } catch (err) {
        console.error("❌ CRITICAL REJECTION LOGGED BY DATABASE ENGINE:");
